 ...
 * <p/>
 * 1. 这里主要将sql对象拆分成三个部分 : 主要是为了满足jdbcTemplate对象的赋值,ab是为了拼装sql能够更加方便,c是根据ab得到的有效值
 * a. whereSql	- 查询条件
 * b. orderSql	- 排序字段
 * c. valueArr - 值对象
 *
...
public class QueryRuleSqlBulider {
    ...
    private void processLike(QueryRule.Rule rule) {
        if (ObjectUtils.isEmpty(rule.getValues())) {
            return;
        }
    ...
    private void processEqual(QueryRule.Rule rule) {
        if (ObjectUtils.isEmpty(rule.getValues()) || rule.getValues()[0] == null) {
            return;
        }
    ...
    private void inAndNotIn(QueryRule.Rule rule, String name) {
        ...
        if (org.springframework.util.ObjectUtils.isEmpty(rule.getValues())) {
            return;
        }
