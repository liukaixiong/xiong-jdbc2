...
import com.x.jdbc.template.common.Page;
import com.x.jdbc.template.common.utils.BeanUtils;
import com.x.jdbc.template.common.utils.DataUtils;
import com.x.jdbc.template.common.utils.GenericsUtils;
...
public abstract class BaseDaoSupport<T extends Serializable, PK extends Serializable>{
    ...
    private Page simplePageQueryNotT(String sql, RowMapper rm, Map<String, ?> args, long pageNo, long pageSize) {
        ...
        long start = (pageNo - 1) * pageSize;
        sql = sql + " limit " + start + "," + pageSize;
        log.debug(com.x.jdbc.template.common.utils.StringUtils.format("[Execute SQL]sql:{0},params:{1}", sql, args));
...
//			return clazz.cast(key);
    ...
    private Map<String, Object> convertMap(Object obj) {
        ...
        for (int i = 0, len = getters.size(); i < len; i++) {
            ...
            String name = fieldInfo.getFormat();
            try {
